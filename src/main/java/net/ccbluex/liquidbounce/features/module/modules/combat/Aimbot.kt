...
object Aimbot : Module("Aimbot", Category.COMBAT) {
    ...
    private fun findRotation(entity: Entity, random: Random): Boolean {
        ...
        fun clamp(v: Double, min: Double, max: Double) = if (v < min) min else if (v > max) max else v

        val eye: Vec3 = player.getPositionEyes(1.0f)
        val cx = clamp(eye.xCoord, boundingBox.minX, boundingBox.maxX)
        val cy = clamp(eye.yCoord, boundingBox.minY, boundingBox.maxY)
        val cz = clamp(eye.zCoord, boundingBox.minZ, boundingBox.maxZ)
        val closestPoint = Vec3(cx, cy, cz)

        val playerRotation = player.rotation
        val destinationRotation = toRotation(closestPoint, true)
        ...
        val supposedTurnSpeed = if (rotationDiff < mc.gameSettings.fovSetting) inViewMaxAngleChange else maxAngleChange
